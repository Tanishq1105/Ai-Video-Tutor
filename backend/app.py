...
def upload_video():
    ...
    if youtube_url:
        ...
        if download_youtube_video(youtube_url, save_path):
            ...
            s3_bucket = os.getenv('SUPABASE_BUCKET_NAME')
            
            if s3_bucket:
                # Upload to Supabase Stoage
                ...
                with open(save_path, 'rb') as f:
                    if upload_to_s3(f, s3_bucket, s3_key, content_type='video/mp4'):
                        ...
                        flash('YouTube video processed and uploaded to Supabase Storage!')
                        return redirect(url_for('index'))
                    else:
                        flash('Failed to upload YouTube video to Supabase Storage')
    ...
    if file:
        ...
        if s3_bucket:
            ...
            if upload_to_s3(file, s3_bucket, s3_key, content_type=file.content_type):
                ...
                flash('Video uploaded to Supabase Storage successfully!')
                return redirect(url_for('index'))
            else:
                flash('Failed to upload to Supabase Storage')
                return redirect(request.url)
        else:
...
def view_video(video_id):
    ...
    if video.s3_key:
        ...
        if s3_bucket:
            ...
            print(f"Generated Supabase Storage URL for video {video_id}: {video_url}")
